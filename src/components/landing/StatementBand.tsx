...
export function StatementBand() {
  return (
    <section className="band grain py-20 md:py-32">
      <div className="mx-auto max-w-[1600px] px-6 md:px-10">
        ...
        <AnimateIn>
          <div className="mt-16 md:mt-24 flex items-center justify-between gap-6 border-b border-white/15 pb-4">
          ...
          {quotes.map((q, i) => {
            ...
            return (
              <figure
                key={q.author.name + i}
                className={cn(
                  "flex min-h-[300px] md:min-h-[360px] w-[min(560px,85vw)] shrink-0 snap-start flex-col rounded-2xl p-6 md:p-8",
                  lightCard
                    ? "bg-[#F4F4F2] text-neutral-900"
                    : "bg-white/[0.06] text-white",
                )}
              >
                <blockquote className="text-xl md:text-2xl leading-snug tracking-[-0.01em]">
                  &ldquo;{q.text}&rdquo;
                </blockquote>
                <figcaption className="mt-auto flex items-center gap-3 pt-8 md:pt-10">
